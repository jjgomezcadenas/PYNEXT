...
def test_pv_body(next100pv):
    ...
    pi * ((pv.radius + pv.body_thickness)**2 - pv.radius**2) * pv.length, rel=1e-5)
    ...
    2 * pi * (pv.radius + pv.body_thickness) * pv.length, rel=1e-5)
...
def test_activity_self_shield_pv_body(next100pv):
    pv = next100pv
    activity_bi214 = pv.body_surface * pv.cv.body.material.surface_activity(pv.cvd.th_body,
                                                                       isotope='Bi214')
    activity_tl208 = pv.body_surface * pv.cv.body.material.surface_activity(pv.cvd.th_body,
                                                                       isotope='Tl208')
    assert pv.body_self_shield_activity_bi214  == approx(activity_bi214, rel=1e-5)
    assert pv.body_self_shield_activity_tl208  == approx(activity_tl208, rel=1e-5)


def test_activity_self_shield_pv_head(next100pv):
    pv = next100pv
    activity_bi214 = pv.head_surface * pv.cv.head.material.surface_activity(pv.cvd.th_head,
                                                                       isotope='Bi214')
    activity_tl208 = pv.head_surface * pv.cv.head.material.surface_activity(pv.cvd.th_head,
                                                                       isotope='Tl208')
    assert pv.head_self_shield_activity_bi214  == approx(activity_bi214, rel=1e-5)
    assert pv.head_self_shield_activity_tl208  == approx(activity_tl208, rel=1e-5)
